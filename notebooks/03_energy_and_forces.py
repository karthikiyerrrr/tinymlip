...
def _(cutoff, graph, mo, n_layers, torch):
    ...
    x_emb = model.embed(graph.z)  # [N, F]  atomic-number embeddings
    x_int = x_emb
    for layer in model.interactions:
        x_int = layer(x_int, graph)  # [N, F]  after message passing
    per_atom_e = model.readout(x_int).squeeze(-1)  # [N]   per-atom scalar energy
    energy = per_atom_e.sum()  # []    molecular energy
    ...
    mo.vstack(
        [
            mo.md(
                f"**Pipeline shapes** (one layer per row):\n\n"
                f"- embedding(z): `{tuple(x_emb.shape)}`\n"
                f"- after {n_layers.value} interaction(s): `{tuple(x_int.shape)}`\n"
                f"- readout: `{tuple(per_atom_e.shape)}`\n"
                f"- sum: `{tuple(energy.shape)}` → scalar **E_total = {e_total:.4f}**"
            ),
            fig_per_atom,
        ]
    )
    return InvariantMPNN, chemical_symbols, go, model
...
def _(atoms, build_graph, cutoff, graph, mo, model, torch):
    ...
    # Build two copies of ethanol, 20 A apart along x. At cutoff=5 A, no edges
    # between the copies — they're independent.
    pos_single = atoms.get_positions()
    shift = np.array([20.0, 0.0, 0.0])
    pos_dimer = np.concatenate([pos_single, pos_single + shift], axis=0)
    numbers_dimer = np.concatenate([atoms.numbers, atoms.numbers], axis=0)
    atoms_dimer = ase.Atoms(numbers=numbers_dimer, positions=pos_dimer)
    graph_dimer = build_graph(atoms_dimer, cutoff=cutoff.value)

    with torch.no_grad():
        e_single = model(graph).item()
        e_dimer = model(graph_dimer).item()

    ratio = e_dimer / e_single if abs(e_single) > 1e-12 else float("nan")
    delta = e_dimer - 2 * e_single

    mo.md(
        f"**E(ethanol)** = `{e_single:.6f}`\n\n"
        f"**E(two ethanols, 20 Å apart)** = `{e_dimer:.6f}`\n\n"
        f"**ratio E_dimer / E_single** = `{ratio:.6f}` (target: exactly 2)\n\n"
        f"**E_dimer − 2·E_single** = `{delta:.2e}`\n\n"
        f"That delta is essentially zero (machine precision in float32). The "
...
def _(
    ...
    # Re-build a graph whose positions are an autograd leaf so we can take
    # d(energy)/d(pos). We don't mutate `graph` (the original) so the upstream
    # cells stay clean.
    graph_f = build_graph(atoms, cutoff=cutoff.value)
    graph_f.pos.requires_grad_(True)
    energy_f = model(graph_f)
    forces = compute_forces(energy_f, graph_f.pos)

    pos_np = graph_f.pos.detach().numpy()
    forces_np = forces.detach().numpy()
    z_np = graph_f.z.numpy()

    # Plotly Cone takes a base position (x, y, z) and a vector (u, v, w).
    # We scale the vectors so they're visible on the molecule's length scale.
    scale = float(arrow_scale.value)
    u = forces_np[:, 0] * scale
    v = forces_np[:, 1] * scale
    w = forces_np[:, 2] * scale
    ...
    fig_forces.add_trace(
        go.Scatter3d(
            ...
            mode="markers+text",
            marker=dict(
                size=[element_radius(int(zi)) * 14 for zi in z_np],
                color=[element_color(int(zi)) for zi in z_np],
                line=dict(color="#222", width=1),
            ),
            text=[f"{chemical_symbols[int(zi)]}[{i}]" for i, zi in enumerate(z_np)],
    ...
    # Force arrows as Plotly Cones (vector glyphs at atom centers).
    fig_forces.add_trace(
        go.Cone(
            x=pos_np[:, 0],
            y=pos_np[:, 1],
            ...
            colorscale="Reds",
            showscale=False,
            hovertemplate=("atom %{customdata}<br>|F|·scale = %{norm:.3f}<extra></extra>"),
            customdata=list(range(graph_f.n_atoms)),
            name="forces",
        )
    )

    max_force = float(forces.detach().norm(dim=-1).max())
    fig_forces.update_layout(
        title=(
            f"Autograd forces on ethanol (untrained, h=32, layers={n_layers.value}). "
            f"|F|_max = {max_force:.4f} · arrows scaled by {scale:.0f}"
        ),
        scene=dict(
    ...
    fig_forces
    return compute_forces, energy_f, forces
...
def _(
    ...
    shift_vec = np.array([1.2, -0.7, 2.4])
    atoms_shifted = atoms.copy()
    atoms_shifted.set_positions(atoms_shifted.get_positions() + shift_vec)

    graph_shifted = build_graph(atoms_shifted, cutoff=cutoff.value)
    graph_shifted.pos.requires_grad_(True)
    energy_shifted = model(graph_shifted)
    forces_shifted = compute_forces(energy_shifted, graph_shifted.pos)

    delta_e = (energy_shifted - energy_f).abs().item()
    sum_forces_orig = forces.detach().sum(dim=0).norm().item()
    sum_forces_shift = forces_shifted.detach().sum(dim=0).norm().item()

    mo.md(
        f"### Translation invariance\n\n"
        f"Shifting all positions by `{shift_vec.tolist()}` Å:\n\n"
        f"- `|E(shifted) − E(original)|` = `{delta_e:.2e}` ✅ (should be ~0)\n\n"
        f"### Force conservation (Σ F ≈ 0)\n\n"
        f"- original geometry: `||Σ F_i||` = `{sum_forces_orig:.2e}` ✅\n"
        f"- shifted geometry:  `||Σ F_i||` = `{sum_forces_shift:.2e}` ✅\n\n"
...
def _(
    ...
    # Autograd force at (i, ax).
    g_auto = build_graph(atoms, cutoff=cutoff.value, dtype=torch.float64)
    g_auto.pos.requires_grad_(True)
    e_auto = model64(g_auto)
    f_auto = compute_forces(e_auto, g_auto.pos)
    autograd_force = f_auto[i, ax].item()

    # Central differences. Rebuild the graph each time so connectivity is
    # always recomputed from the perturbed positions (won't change at this eps,
    # but the pattern matches the pytest version under tests/test_forces.py).
    def _e_at(delta):
        a = atoms.copy()
        p = a.get_positions()
        p[i, ax] += delta
        a.set_positions(p)
        g = build_graph(a, cutoff=cutoff.value, dtype=torch.float64)
        with torch.no_grad():
            return model64(g).item()

    e_plus = _e_at(+eps)
    e_minus = _e_at(-eps)
    numerical_force = -(e_plus - e_minus) / (2 * eps)
    abs_err = abs(numerical_force - autograd_force)

    axis_label = {0: "x", 1: "y", 2: "z"}[ax]
    mo.md(
        f"With `ε = {eps}` Å, atom **{i}** ({chemical_symbols[int(graph.z[i])]}), axis **{axis_label}**:\n\n"
        f"- autograd `F[{i}, {axis_label}]`     = `{autograd_force:+.8f}`\n\n"
        f"- numerical `−(E₊ − E₋)/(2ε)` = `{numerical_force:+.8f}`\n\n"
        f"- `|autograd − numerical|`     = `{abs_err:.2e}`\n\n"
